  ...
  describe '.methods' do
    it 'by_file' do
      expect(Cnc::Storage.by_file('spec/fixtures/cnc-storage.png')).to(
        match_array([
                      URI.join(Cnc::Storage::Configuration.default.cdn_url, 'cnc-storage.png')
                    ])
      )
    end

    it 'by_file with variants' do
      expect(Cnc::Storage.by_file('spec/fixtures/cnc-storage.png', variants: %i[thumbnail small])).to(
        match_array([
                      URI.join(Cnc::Storage::Configuration.default.cdn_url, 'thumbnail_cnc-storage.png'),
                      URI.join(Cnc::Storage::Configuration.default.cdn_url, 'small_cnc-storage.png'),
                      URI.join(Cnc::Storage::Configuration.default.cdn_url, 'cnc-storage.png')
                    ])
      )
    end
  ...
  describe 'options' do
    context 'when original is set to false' do
      it 'does not return any image' do
        expect(Cnc::Storage.by_file('spec/fixtures/cnc-storage.png', original: false)).to(
          match_array([])
        )
      end
    end
    ...
    context 'when extension is set to webp' do
      it 'returns an image with webp format' do
        expect(Cnc::Storage.by_file('spec/fixtures/cnc-storage.png', extension: 'webp')).to(
          match_array([
                        URI.join(Cnc::Storage::Configuration.default.cdn_url, 'cnc-storage.webp')
                      ])
        )
      end
    end
